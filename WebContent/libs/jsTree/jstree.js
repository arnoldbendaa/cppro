...
(function(factory) {
	...
	$.jstree.core.prototype = {
		...
		check: function(chk, obj, par, pos, more) {
			...
			var objParent = this.get_node(obj.parent);
			var tmp = chk.match(/^move_node|copy_node|create_node$/i) ? par : obj,
				chc = this.settings.core.check_callback;

			// START OUR CHANGES
			if (chk === "move_node") {	
				// Root - block any move action
				if (obj.id === "#" || par.id === "#") {
					return false;
				}

				// Feeder elements (non augment) may not contain any child elements			
				if (more.pos === "i" && par.original.augentElement === false && par.parents.length > 1) {
					return false;
				}
				if (more.pos === undefined && obj.parent === par.parent && par.original.augentElement === false && par.parents.length > 1) {
					return false;
				}
				if ((more.pos === "i" || more.pos === undefined) && par.state.leaf === true && par.original.augentElement === false && par.parents.length > 1) {
					return false;
				}

				if (more.pos === "i" && obj.state.leaf === true && obj.original.augentElement === false) {
					return false;
				}
				if (more.pos === undefined && obj.state.leaf === true && obj.original.augentElement === false && obj.parent !== par.id) {
					return false;
				}
				if (more.pos === "i" && par.original.type !== "directory" && par.original.type !== undefined && obj.parent !== par.id) {
                    return false;
                }
			}
			// END OUR CHANGES

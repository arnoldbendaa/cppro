...
function bindEvent(spread){
    spread.bind(GC.Spread.Sheets.Events.ActiveSheetChanged, function (sender, args) {
        getChartData(spread.getActiveSheet().toJSON());
        spreadEnable(spread);
        spread.getActiveSheet().bind(GC.Spread.Sheets.Events.EditChange, function (sender, args) {
            var json = spread.getActiveSheet().toJSON();
            json.data.dataTable[args.row][args.col].value=parseInt(args.editingText);
            var selection = spread.getActiveSheet().getSelections();
            drawSelection(json,selection);
        });
        spread.getActiveSheet().bind(GC.Spread.Sheets.Events.SelectionChanged, function (sender, args) {
            var json = spread.getActiveSheet().toJSON();
            var selection = spread.getActiveSheet().getSelections();
            drawSelection(json,selection);
        });
    ...
    function getChartData(json) {
        ...
        if(sheet.name!="Evaluation Version"){
            ...
            var rowLen = json.rowCount;
            var colLen = json.columnCount;
            var datas = [];
                    ...
                    if(ExcelDatas[i]!=undefined&&ExcelDatas[i]!=""){
                        if(ExcelDatas[i][j]!=undefined&&ExcelDatas[i][j]!=""){
                            if(ExcelDatas[i][j].value!=undefined&&ExcelDatas[i][j].value!="") {
                                datas[j].data.push(ExcelDatas[i][j].value);
                            }
                        }
                    }
                }
...
//            var colLen = json.columnCount;
    ...
    function changeFile(){
        ...
        $.ajax({
        	url:localStorage["cp_base_url"]+"openExcel",
        	type:"POST",
        	data:{
        		fileName:fileName
        	},
...
function saveToPng(){
	html2canvas(document.body).then(function(canvas) {
		...
		var fileName = $("#activeProfieSelectBox").val();
		waitingDialog.show("saving to PPT");
        $.ajax({
...
function exportPptFile(){
	var fileName = $("#activeProfieSelectBox").val();
	var pptUrl = localStorage["cp_base_url"]+"/uploadFiles/"+fileName+".pptx";
    var a = document.createElement("a");
    a.href = pptUrl;
    document.body.appendChild(a);
    a.click();

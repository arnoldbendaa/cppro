package com.cedar.cp.ejb.impl.recalculate;

import com.cedar.cp.api.base.CPException;
import com.cedar.cp.api.base.DuplicateNameValidationException;
import com.cedar.cp.api.base.ValidationException;
import com.cedar.cp.dto.recalculate.RecalculateBatchTaskCK;
import com.cedar.cp.dto.recalculate.RecalculateBatchTaskPK;
import com.cedar.cp.ejb.impl.recalculate.RecalculateBatchTaskDAO;
import com.cedar.cp.ejb.impl.recalculate.RecalculateBatchTaskEVO;
import com.cedar.cp.util.Log;
import com.cedar.cp.util.Timer;
import javax.ejb.CreateException;
import javax.ejb.EJBException;
import javax.ejb.EntityBean;
import javax.ejb.EntityContext;
import javax.ejb.FinderException;
import javax.ejb.RemoveException;

public class RecalculateBatchTaskEEJB implements EntityBean {

   protected EntityContext mEntityContext;
   private RecalculateBatchTaskDAO mRecalculateBatchTaskDAO;
   private transient Log mLog = new Log(this.getClass());


   public RecalculateBatchTaskPK ejbCreate(RecalculateBatchTaskEVO details) throws CreateException, EJBException {
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;

      RecalculateBatchTaskPK var4;
      try {
         RecalculateBatchTaskDAO e = this.getDAO();
         e.setDetails(details);
         var4 = e.create();
      } catch (DuplicateNameValidationException var9) {
         throw new EJBException(var9);
      } catch (ValidationException var10) {
         throw new EJBException(var10);
      } finally {
         if(timer != null) {
            timer.logDebug("ejbCreate", "");
         }

      }

      return var4;
   }

   public void ejbPostCreate(RecalculateBatchTaskEVO details) throws CreateException, EJBException {}

   public void ejbRemove() throws RemoveException {
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;

      try {
         this.getDAO().remove(this.getPK());
      } catch (Exception var6) {
         throw new CPException("unable to remove " + this.getPK());
      } finally {
         if(timer != null) {
            timer.logDebug("ejbRemove", "");
         }

      }

   }

   public RecalculateBatchTaskPK ejbFindByPrimaryKey(RecalculateBatchTaskPK pk) throws FinderException {
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;

      RecalculateBatchTaskPK ve;
      try {
         ve = this.getDAO().findByPrimaryKey(pk);
      } catch (ValidationException var7) {
         throw new CPException(var7.getMessage(), var7);
      } finally {
         if(timer != null) {
            timer.logDebug("ejbFindByPrimaryKey", pk.toString());
         }

      }

      return ve;
   }

   public void ejbLoad() {
      RecalculateBatchTaskPK pk = (RecalculateBatchTaskPK)this.mEntityContext.getPrimaryKey();
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;

      try {
         this.getDAO().load(pk);
      } catch (ValidationException var7) {
         throw new EJBException(var7);
      } finally {
         if(timer != null) {
            timer.logDebug("ejbLoad", pk.toString());
         }

      }

   }

   public void ejbStore() throws EJBException {
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;

      try {
         this.getDAO().store();
      } catch (EJBException var7) {
         this.mLog.error("ejbStore", "failed", var7);
         throw var7;
      } catch (Exception var8) {
         this.mLog.error("ejbStore", "failed", var8);
         throw new EJBException(var8);
      } finally {
         if(timer != null) {
            timer.logDebug("ejbStore", this.getPK().toString());
         }

      }

   }

   private RecalculateBatchTaskPK getPK() {
      return (RecalculateBatchTaskPK)this.mEntityContext.getPrimaryKey();
   }

   public void ejbActivate() {}

   public void ejbPassivate() {
      this.mLog.debug("ejbPassivate", this.getPK().toString());
      this.mRecalculateBatchTaskDAO = null;
   }

   public void setEntityContext(EntityContext entityContext) {
      this.mEntityContext = entityContext;
   }

   public void unsetEntityContext() {
      this.mEntityContext = null;
   }

   protected RecalculateBatchTaskDAO getDAO() {
      if(this.mRecalculateBatchTaskDAO == null) {
         this.mRecalculateBatchTaskDAO = new RecalculateBatchTaskDAO();
      }

      return this.mRecalculateBatchTaskDAO;
   }

   public RecalculateBatchTaskEVO getDetails(RecalculateBatchTaskCK paramCK, String dependants) throws ValidationException {
      return this.getDAO().getDetails(paramCK, dependants);
   }

   public RecalculateBatchTaskEVO getDetails(String dependants) throws ValidationException {
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;

      RecalculateBatchTaskEVO evo;
      try {
         evo = this.getDAO().getDetails(dependants);
      } finally {
         if(timer != null) {
            timer.logDebug("getDetails", dependants);
         }

      }

      return evo;
   }

   public RecalculateBatchTaskEVO getDetails(RecalculateBatchTaskCK paramCK) throws ValidationException {
//      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;
//
//      RecalculateBatchTaskEVO var3;
//      try {
//         var3 = this.getDAO().getDetails(paramCK, "");
//      } finally {
//         if(timer != null) {
//            timer.logDebug("getDetails", paramCK.toString());
//         }
//
//      }

      return null;
   }

   public void setDetails(RecalculateBatchTaskEVO details) {
      this.getDAO().setDetails(details);
   }

   public RecalculateBatchTaskEVO setAndGetDetails(RecalculateBatchTaskEVO evo, String dependants) {
      return this.getDAO().setAndGetDetails(evo, dependants);
   }
}

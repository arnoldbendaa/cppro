// Decompiled by:       Fernflower v0.8.6
// Date:                12.08.2012 13:05:10
// Copyright:           2008-2012, Stiver
// Home page:           http://www.neshkov.com/ac_decompiler.html

package com.cedar.cp.impl.currency;

import com.cedar.cp.api.base.CPConnection;
import com.cedar.cp.api.base.CPException;
import com.cedar.cp.api.base.EntityList;
import com.cedar.cp.api.base.ValidationException;
import com.cedar.cp.api.currency.CurrencyEditorSession;
import com.cedar.cp.api.currency.CurrencysProcess;
import com.cedar.cp.ejb.api.currency.CurrencyEditorSessionServer;
import com.cedar.cp.impl.base.BusinessProcessImpl;
import com.cedar.cp.impl.currency.CurrencyEditorSessionImpl;
import com.cedar.cp.util.Log;
import com.cedar.cp.util.Timer;

public class CurrencysProcessImpl extends BusinessProcessImpl implements CurrencysProcess {

   private Log mLog = new Log(this.getClass());


   public CurrencysProcessImpl(CPConnection connection) {
      super(connection);
   }

   public void deleteObject(Object primaryKey) throws ValidationException {
      Timer timer = this.mLog.isDebugEnabled()?new Timer(this.mLog):null;
      CurrencyEditorSessionServer es = new CurrencyEditorSessionServer(this.getConnection());

      try {
         es.delete(primaryKey);
      } catch (ValidationException var5) {
         throw var5;
      } catch (CPException var6) {
         throw new RuntimeException("can\'t delete " + primaryKey, var6);
      }

      if(timer != null) {
         timer.logDebug("deleteObject", primaryKey);
      }

   }

   public CurrencyEditorSession getCurrencyEditorSession(Object key) throws ValidationException {
      CurrencyEditorSessionImpl sess = new CurrencyEditorSessionImpl(this, key);
      this.mActiveSessions.add(sess);
      return sess;
   }

   public EntityList getAllCurrencys() {
      try {
         return this.getConnection().getListHelper().getAllCurrencys();
      } catch (Exception var2) {
         var2.printStackTrace();
         throw new RuntimeException("can\'t get AllCurrencys", var2);
      }
   }

   public String getProcessName() {
      String ret = "Processing Currency";
      return ret;
   }

   protected int getProcessID() {
      return 18;
   }
}

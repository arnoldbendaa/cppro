...
public class MaxaFunctionExecutor extends FunctionExecutor {

   private boolean mFirstValue;


   public MaxaFunctionExecutor() {
      this.setName("MAXA");
   }

   public Object execute(List params) {
      if(params == null) {
         throw new IllegalStateException("Wrong number of parameters to function MAXA()");
      } else if(params.size() > 30) {
         throw new IllegalStateException("Too many arguments to function MAXA(). Maximum of 30 arguments");
      } else {
            ...
            if(param instanceof CellRef) {
               ...
               if(cell != null && cell.getStringValue() != null && !cell.getStringValue().equals("")) {
                  if(!"TRUE".equals(cell.getStringValue().toUpperCase()) && !"FALSE".equals(cell.getStringValue().toUpperCase())) {
                     ...
                     maxValue = (double)(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1:0);
                     this.mFirstValue = false;
                  } else {
                     maxValue = Math.max(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1.0D:0.0D, maxValue);
                  }
               }
               ...
               if(param instanceof String) {
                  ...
                  if(!(param instanceof Boolean)) {
                     throw new IllegalArgumentException("Unexpected parameter " + param + " in MAXA() function");
                  }
      ...
      while(cellIter.hasNext()) {
         ...
         if(cell != null && cell.getStringValue() != null && !cell.getStringValue().equals("")) {
            if(!"TRUE".equals(cell.getStringValue().toUpperCase()) && !"FALSE".equals(cell.getStringValue().toUpperCase())) {
               ...
               result = Math.max(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1.0D:0.0D, result);
            }
         }
      }

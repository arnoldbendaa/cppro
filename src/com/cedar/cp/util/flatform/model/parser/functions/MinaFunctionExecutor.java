...
public class MinaFunctionExecutor extends FunctionExecutor {

   private boolean mFirstValue;


   public MinaFunctionExecutor() {
      this.setName("MINA");
   }

   public Object execute(List params) {
      if(params == null) {
         throw new IllegalStateException("Wrong number of parameters to function MINA()");
      } else if(params.size() > 30) {
         throw new IllegalStateException("Too many arguments to function MINA(). Maximum of 30 arguments");
      } else {
         ...
         while(i$.hasNext()) {
            ...
            if(param instanceof CellRef) {
               ...
               if(cell != null && cell.getStringValue() != null && !cell.getStringValue().equals("")) {
                  if(!"TRUE".equals(cell.getStringValue().toUpperCase()) && !"FALSE".equals(cell.getStringValue().toUpperCase())) {
                  ...
                  } else if(this.mFirstValue) {
                     minValue = (double)(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1:0);
                     this.mFirstValue = false;
                  } else {
                     minValue = Math.min(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1.0D:0.0D, minValue);
                  }
               }
               ...
               if(param instanceof String) {
                  ...
                  if(!(param instanceof Boolean)) {
                     throw new IllegalArgumentException("Unexpected parameter " + param + " in MINA() function");
                  }
      ...
      while(cellIter.hasNext()) {
         ...
         if(cell != null && cell.getStringValue() != null && !cell.getStringValue().equals("")) {
            if(!"TRUE".equals(cell.getStringValue().toUpperCase()) && !"FALSE".equals(cell.getStringValue().toUpperCase())) {
               ...
               result = (double)(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1:0);
               this.mFirstValue = false;
            } else {
               result = Math.min(Boolean.valueOf(cell.getStringValue()) == Boolean.TRUE?1.0D:0.0D, result);
            }
         }
      }
